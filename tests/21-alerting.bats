...
@test "--test-alert digest email dispatches on fresh cursor (no prior digest)" {
    # Regression: _test_alert_digest used to call tlog_read with no cursor
    # pre-seeded. Under default TLOG_FIRST_RUN=skip the first-run path wrote
    # a cursor at current file size but returned no content, so the digest
    # bailed early at _hook_hit_count=0 and no email was dispatched — even
    # though the test framework printed "test email digest alert sent".
    # reset-lmd.sh clears $sessdir and $tmpdir between tests, so the setup
    # here is exactly the fresh-install scenario the bug hit.
    create_mock_sendmail
    lmd_set_config email_alert 1
    lmd_set_config email_addr "test@example.com"
    lmd_set_config email_subj "maldet alert"
    lmd_set_config email_format "text"
    run_maldet_with_mocks --test-alert digest email
    assert_success
    assert_output --partial "test email digest alert sent"
    [ -f /tmp/mock-sendmail.body ]
    [ -s /tmp/mock-sendmail.body ]
}

@test "--test-alert digest email body contains both synthetic hit signatures" {
    create_mock_sendmail
    lmd_set_config email_alert 1
    lmd_set_config email_addr "test@example.com"
    lmd_set_config email_format "text"
    ...
    assert_success
    [ -f /tmp/mock-sendmail.body ]
    run grep "php.cmdshell.test.1" /tmp/mock-sendmail.body
    assert_success
    run grep "php.webshell.test.2" /tmp/mock-sendmail.body
    assert_success
}

@test "--test-alert digest email restores hook cursor to pre-test value" {
    # Regression: prior implementation restored cursor_backup only when
    # _had_cursor=1. After the fix, the cursor is seeded to _orig_size before
    # genalert (forcing delta read), advanced past the test hits by tlog_read,
    # then restored to the backup on exit. Verify the real cursor value is
    # untouched post-test.
    create_mock_sendmail
    lmd_set_config email_alert 1
    lmd_set_config email_addr "test@example.com"
    lmd_set_config email_format "text"
    # Seed a real cursor at a non-zero, recognizable position
    mkdir -p "$LMD_INSTALL/tmp"
    echo "42" > "$LMD_INSTALL/tmp/digest.hook.alert"
    # Touch the hook log so _orig_size isn't 0 and the cursor value is plausible
    mkdir -p "$LMD_INSTALL/sess"
    printf 'x%.0s' {1..42} > "$LMD_INSTALL/sess/hook.hits.log"
    run_maldet_with_mocks --test-alert digest email
    assert_success
    [ -f "$LMD_INSTALL/tmp/digest.hook.alert" ]
    cursor_val=$(cat "$LMD_INSTALL/tmp/digest.hook.alert")
    [ "$cursor_val" = "42" ]
}

@test "--test-alert digest email does not leak pre-existing real hook hits" {
    # Regression (S-REG-004 data isolation): with a trailing cursor, the old
    # implementation would send unalerted real hook hits as part of the test
    # email. The seed-cursor-to-orig-size fix restricts the tlog_read delta
    # to only the synthetic rows appended by the test.
    create_mock_sendmail
    lmd_set_config email_alert 1
    lmd_set_config email_addr "test@example.com"
    lmd_set_config email_format "text"
    mkdir -p "$LMD_INSTALL/sess" "$LMD_INSTALL/tmp"
    # Cursor at 0 simulates an unalerted backlog of real hits
    echo "0" > "$LMD_INSTALL/tmp/digest.hook.alert"
    # Realistic-looking pre-existing hit with a distinctive sig string
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        '{HEX}real.production.sig.DO_NOT_LEAK' '/var/www/html/real.php' \
        '-' 'HEX' 'HEX Pattern' '-' '1024' 'web' '33' '644' \
        "$(date +%s)" 'modsec' "$(date +%s)" \
        > "$LMD_INSTALL/sess/hook.hits.log"
    run_maldet_with_mocks --test-alert digest email
    assert_success
    [ -f /tmp/mock-sendmail.body ]
    # The real sig MUST NOT leak into the test email
    run grep "DO_NOT_LEAK" /tmp/mock-sendmail.body
    [ "$status" -ne 0 ]
    # The synthetic test sigs MUST still be present
    run grep "php.cmdshell.test.1" /tmp/mock-sendmail.body
    assert_success
}

@test "--test-alert digest email preserves pre-existing hook.hits.log content" {
    # The cleanup path must restore hook.hits.log to its pre-test state so the
    # real hits seeded above remain available to the next real digest cron run.
    create_mock_sendmail
    lmd_set_config email_alert 1
    lmd_set_config email_addr "test@example.com"
    lmd_set_config email_format "text"
    ...
    echo "0" > "$LMD_INSTALL/tmp/digest.hook.alert"
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        '{HEX}preserved.real.hit.marker' '/var/www/html/real.php' \
        '-' 'HEX' 'HEX Pattern' '-' '1024' 'web' '33' '644' \
        "$(date +%s)" 'modsec' "$(date +%s)" \
        > "$LMD_INSTALL/sess/hook.hits.log"
    orig_size=$(stat -c %s "$LMD_INSTALL/sess/hook.hits.log")
    run_maldet_with_mocks --test-alert digest email
    assert_success
    # Pre-existing content is still present
    run grep "preserved.real.hit.marker" "$LMD_INSTALL/sess/hook.hits.log"
    assert_success
    # File truncated back to original size (synthetic test hits removed)
    new_size=$(stat -c %s "$LMD_INSTALL/sess/hook.hits.log")
    [ "$new_size" = "$orig_size" ]
}
