...
monitor_init() {
	...
	if [ -f "/proc/sys/fs/inotify/max_user_instances" ] && [ -f "/proc/sys/fs/inotify/max_user_watches" ]; then
		...
		users_tot=$(awk -F: -v min="$inotify_minuid" '$3 >= min && $6 ~ /^\/home\// {n++} END {print n+0}' /etc/passwd)
	...
	if [[ "$inopt" =~ ^[Uu][Ss][Ee][Rr][Ss]?$ ]]; then
		while IFS=':' read -r user user_id user_home; do
			icnt=$((icnt + 1))
			if [ -f "${monitor_disabled_users:-}" ] && grep -Fqx -- "$user" "$monitor_disabled_users"; then
				eout "{mon} skipped disabled user $user" 1
				continue
			fi
			if [ "$user_id" -ge "$inotify_minuid" ] && [[ "$user_home" == /home/* ]] && [ -d "$user_home" ]; then
