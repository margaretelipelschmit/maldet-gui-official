...
_alert_telegram_escape() {
	...
	# Preserve escapes already present in templates and escape only unescaped
	# MarkdownV2 punctuation from dynamic values.
	printf '%s' "$1" | awk '{
...
# _alert_redact_url url — return a URL for logging
_alert_redact_url() {
	printf '%s' "$1"
}
...
_alert_mail_command() {
	local _timeout="${ALERT_MAIL_TIMEOUT:-30}" _rc
	case "$_timeout" in
		''|*[!0-9]*) _timeout=30 ;;
	esac
	if command -v timeout >/dev/null 2>&1; then
		timeout --signal=TERM --kill-after=5 "${_timeout}s" "$@"
		_rc=$?
		[ "$_rc" -eq 124 ] && echo "alert_lib: mail command timed out after ${_timeout}s; check the local MTA configuration." >&2
		return "$_rc"
	fi
	"$@"
}
...
_alert_email_local() {
	...
	case "$format" in
		text)
			if [ -n "$mail_bin" ]; then
				_alert_mail_command "$mail_bin" -s "$subject" "$recip" < "$text_file"
				return $?
			fi
			...
			if [ -n "$sendmail_bin" ]; then
				...
				_alert_mail_command "$sendmail_bin" -t -oi < "$_tmpmail"
				local _rc=$?
			...
			if [ -n "$sendmail_bin" ]; then
				{
					...
					# base64 wraps at 76 chars, satisfying RFC 5321 998-char line limit
					base64 < "$html_file"
				} | _alert_mail_command "$sendmail_bin" -t -oi
				return $?
			fi
...
_alert_telegram_message() {
	...
	text=$(_alert_telegram_escape "$text")
	_alert_telegram_api "sendMessage" "$bot_token" \
		--form-string "chat_id=$chat_id" \
		--form-string "text=$text" \
		--form-string "parse_mode=MarkdownV2" > /dev/null
}
